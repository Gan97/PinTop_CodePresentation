...
public class ChangeMottoAty extends BaseActivity {
    @Bind(R.id.btExit)
    ImageView btExit;
    @Bind(R.id.tvTitle)
    TextView tvTitle;
    ...
    public int getContentId() {
        return R.layout.changemotto_aty;
    }

    @Override
    public void initView() {
        etNick.setFilters(new InputFilter[]{new InputFilter.LengthFilter(30)});
    }
        ...
        switch (view.getId()) {
            ...
            case R.id.btSave:
                if (etNick.getText().toString()!=null&&etNick.getText().toString().length()!=0){
                    EventBus.getDefault().post(new ChangeMottoEvent(etNick.getText().toString()));
                    finish();
                }else {
