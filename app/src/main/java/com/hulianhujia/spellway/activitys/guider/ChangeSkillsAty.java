...
public class ChangeSkillsAty extends BaseActivity {
    @Bind(R.id.btExit)
    ImageView btExit;
    @Bind(R.id.tvTitle)
    TextView tvTitle;
    ...
    public int getContentId() {
        return R.layout.activity_change_skills_aty;
    }
        ...
        switch (view.getId()) {
            ...
            case R.id.btSave:
                if (etSkill.getText().length()!=0&&etSkill.getText()!=null){
                    EventBus.getDefault().post(new ChangeSkillsEvent(etSkill.getText().toString()));
                    finish();
                }else {
